...
if __name__ == '__main__':
    ...
    for t in range(num_time_steps):
        x = rho * X[-1] + Gaussian(μ=0, σ=math.sqrt(var_eta)).sample()
        X.append(x)
        

    price_diff = [lambda p_s: 1 - beta * p_s.shares - theta * X[t] for t in range(num_time_steps)]
    dynamics = [lambda p_s: p_s.price * math.exp(Gaussian(
        μ=mu_z,
        σ=math.sqrt(var_z)
